...
function [eta,Yeta] = precond_laplace_overlapGS( L, xi, xL, xR, G, Lapl )
    ...
    Y{1} = tensorprod( xi.U{1}, right, 3 );      

    for idx = 2:d-1
        left = innerprod( xL, xi, 'LR', idx-1 );
        right = innerprod( xR, xi, 'RL', idx+1 ); 
        res = tensorprod( xi.U{idx}, left, 1 );
        Y{idx} = tensorprod( res, right, 3 );
    end 
    
    % contract to last core
    left = innerprod( xL, xi, 'LR', d-1 );
    Y{d} = tensorprod( xi.U{d}, left, 1 );
    ...
    for idx = 2:d-1
        ...
        res = tensorprod( PEta.U{idx}, left, 1 );
        Yeta = tensorprod( res, right, 3 );
    ...
    X_mid.U{d} = tensorprod(X_mid.U{d},G{d-1},1);

    Eta = tangent_to_TTeMPS( eta );

    PEta = xi - apply(Lapl, Eta);
    left = innerprod( xL, PEta, 'LR', d-1 ); 
    Yeta = tensorprod( PEta.U{d}, left, 1 );

    eta.dU{d} = solve_inner( L{d}, X_mid, Yeta, d );  
    
    for idx = d-1:-1:2
        X_mid.U{idx+1} = xR.U{idx+1};
        X_mid.U{idx} = tensorprod(X_mid.U{idx},(G{idx}'),3);
    ...
    X_mid.U{1+1} = xR.U{1+1};
        X_mid.U{1} = tensorprod(X_mid.U{1},(G{1}'),3);
        
        Eta = tangent_to_TTeMPS( eta );
        
        PEta = xi - apply(Lapl, Eta);
        right = innerprod( xR, PEta, 'RL', 2 ); 
        Yeta = tensorprod( PEta.U{1}, right, 3 );  
